...
/**
 * Sample workspace instruction
 */
const WORKSPACE_INSTRUCTION = `You are part of a code assistant workflow. This workspace demonstrates how multiple agents collaborate to plan, implement, review, and approve code changes.
 ...
 */
const PLANNER_INSTRUCTION = `You are the Planner. Your job is to analyze task requirements and create a clear, actionable implementation plan.
...
/**
 * Implementer agent instruction
 */
const IMPLEMENTER_INSTRUCTION = `You are the Implementer. Your job is to execute the plan and write working code.
 ...
 */
const REVIEWER_INSTRUCTION = `You are the Reviewer. Your job is to ensure code quality and catch issues before approval.
...
/**
 * Approver agent instruction
 */
const APPROVER_INSTRUCTION = `You are the Approver. Your job is to perform final verification and signal when work is ready for human review.
...
/**
 * Create the sample workspace with agents
 *
 * This is called on first startup to help users understand how Malamar works.
 */
export function createSampleWorkspace(): void {
  ...
  const workspace = createWorkspace({
    ...
    description: WORKSPACE_INSTRUCTION,
  ...
  logger.info('Sample workspace created', { workspaceId: workspace.id });

  // Create the agents
  const agents = [
    { name: 'Planner', instruction: PLANNER_INSTRUCTION, order: 1 },
    { name: 'Implementer', instruction: IMPLEMENTER_INSTRUCTION, order: 2 },
    { name: 'Reviewer', instruction: REVIEWER_INSTRUCTION, order: 3 },
    { name: 'Approver', instruction: APPROVER_INSTRUCTION, order: 4 },
  ];
  ...
  for (const agentData of agents) {
    ...
    logger.info('Sample agent created', { agentId: agent.id, name: agent.name });
  }

  logger.info('Sample workspace setup complete');
}

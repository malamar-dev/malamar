...
export function resolveBinaryPath(): string | null {
  ...
  if (cliSettings.gemini?.binaryPath) {
    return cliSettings.gemini.binaryPath;
  }
...
async function getVersion(
  ...
  try {
    ...
    // Parse version from output (e.g., "0.25.2")
    if (versionLine) {
...
export const geminiAdapter: CliAdapter = {
  ...
  async healthCheck(signal?: AbortSignal): Promise<CliHealthResult> {
    ...
    try {
      ...
      // Gemini CLI uses positional prompt instead of --prompt flag
      const proc = Bun.spawn([binaryPath, CLI_HEALTH_CHECK_PROMPT], {
        cwd: tempDir,
        stdout: "pipe",
        stderr: "pipe",
      });
